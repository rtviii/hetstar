...
import { residueKey, type ChainSequence } from "@dynamic-pdb/hetkit/model";
import { hexCss, rampCss } from "@/lib/lab/color";
import { MODEL_A_COLOR } from "@/lib/molstar/style";
import type { LaneContext, LaneMetric, LaneModule, LaneProps } from "./types";

// Per-position numeric lanes drawn on a canvas: the conformer count of model A, and one
// lane per metric (generated by makeMetricLane; the host decides which metrics exist and
// computes their tracks). All take a Float32Array over positions (NaN = no value); the
// shared draw routine handles bars, baselines and ticks. Metric names live in the row
// label, not on the canvas.
...
const ConformerLaneView = memo(function ConformerLaneView({ ctx, view }: LaneProps) {
  ...
  // keyed on the two fields it reads, not the whole ctx (which changes with color-by)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const { counts, max } = useMemo(() => conformerCounts(ctx), [ctx.chain, ctx.confPlan]);
...
// --- metric lanes: one module per metric id, generated for the host ---

// key -> track row index, built once per track (tracks are immutable once computed)
const trackIndexCache = new WeakMap<Track, Map<string, number>>();
function trackIndex(track: Track): Map<string, number> {
  let idx = trackIndexCache.get(track);
  if (!idx) {
    const built = new Map<string, number>();
    track.keys.forEach((k, i) => built.set(residueKey(k), i));
...
/** the metric's values laid over the chain's positions (NaN = no value there) */
export function metricValues(chain: ChainSequence, m: LaneMetric): Float32Array {
  const idx = trackIndex(m.track);
  const out = new Float32Array(chain.length);
  chain.positions.forEach((p, i) => {
    const j = p.ref ? idx.get(residueKey(p.ref)) : undefined;
    out[i] = j === undefined ? NaN : m.track.values[j];
  });
  return out;
}

function metricLaneView(id: string) {
  return memo(function MetricLaneView({ ctx, view }: LaneProps) {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const entry = ctx.metrics.get(id);
    const metric = entry && typeof entry !== "string" ? entry : null;
    const values = useMemo(() => (metric ? metricValues(ctx.chain, metric) : null), [ctx.chain, metric]);
...
/** one lane per metric; the host builds these from its metric list, once */
export function makeMetricLane(id: string, label: string, description: string): LaneModule {
  return {
    id: `metric:${id}`,
    label,
    description,
    ...
    height: METRIC_H,
    unavailable: (ctx) => {
      const m = ctx.metrics.get(id);
      return m === undefined ? "load a model" : typeof m === "string" ? m : null;
    },
    Component: metricLaneView(id),
    readout: (ctx, pos) => {
      const entry = ctx.metrics.get(id);
      if (!entry || typeof entry === "string") return null;
      const p = ctx.chain.positions[pos - 1];
      if (!p?.ref) return null;
      const i = trackIndex(entry.track).get(residueKey(p.ref));
      if (i === undefined || Number.isNaN(entry.track.values[i])) return null;
      return `${label} ${entry.track.values[i].toFixed(2)}${entry.unit ? ` ${entry.unit}` : ""}`;
    },
  };
}
