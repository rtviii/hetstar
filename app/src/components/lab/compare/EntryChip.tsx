...
export default function EntryChip({
  ...
  const identity = entry ? (
    <>
      ...
      <span className="text-ink-muted">{entry.source === "dpdb" ? entry.id : "bundled"}</span>
    </>
  ) : (
  ...
  const popover = (
    <div className="flex flex-col gap-2">
      ...
      <TinyText className="border-t border-line pt-1.5">
        bundled: {ENTRIES.map((e) => e.id).join(", ")} (their qFit models ship with the app). Dynamic PDB catalogue:
