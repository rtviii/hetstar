...
// from hetkit's METRICS registry (the single source of truth); this module only adds the
// rendering-side fields (ramps, input requirements, toolbar text) and the provenance-
// interpolated tooltip content. The one composite is altloc-rmsf-delta, which the app
// builds from two altloc-rmsf runs via trackDelta.

export type MetricId = "model-rmsd" | "altloc-rmsf-delta" | "fofc-mean" | "fofc-peak" | "support-delta";
...
export interface MetricUi {
  /** short text on the toolbar switch button */
  button: string;
  /** needs model B */
  pair: boolean;
...
export const METRIC_UI: Record<MetricId, MetricUi> = {
  "model-rmsd": {
    ...
    unionSplat: true,
    sequential: true,
    colors: [0xf0f0f0, 0xfdae61, 0xd7191c],
  },
  "altloc-rmsf-delta": {
    button: "RMSF delta",
    pair: true,
